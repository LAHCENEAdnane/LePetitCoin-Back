...
router.post("/", (req, res) => {
  try {
    if (
      ...
      res.json({
        result: false,
        error: "Remplissez tous les champs de saisie",
      });
      return;
    }
    ...
    res
      .status(500)
      .json({
        error: "Une erreur est survenue lors de l'enregistrement de la Toilet.",
      });
  }
});
...
router.get("/", async (req, res) => {
  ...
  Toilet.find({ commune: { $regex: new RegExp(req.query.commune, "i") } })
    .then((data) => {
      if (data === null) {
...
router.get("/:id", (req, res) => {
  ...
  Toilet.findOne({ _id: toiletId }).then((data) => {
    console.log(data);
  });
});
